...
const CartClient: React.FC<CartClientProps> = ({currentUser}) => {
    ...
    return ( 
        <div>
            ...
            <div className="border-t-[1.5px] border-slate-200 flex justify-between gap-4 mt-1">
                <div className="w-[90px] mt-4">
                    <Button label="Clear Cart" onClick={() => {handleClearCart()}} small outline />
                </div>
                <div className="text-sm flex flex-col gap-1 items-start mt-4">
                    <div className="flex justify-between w-full text-base font-semibold">
                        <span>Subtotal</span>
                        <span>{formatPrice(cartTotalAmt)}</span>
                    </div>
                    <p className="text-slate-500 text-xs">Taxes and shipping calculate at checkout</p> 
